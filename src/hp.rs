...
/*
    #[test]
    fn vec_u8_int() {
        let mut int_lis: Hp<Vec<int>> = Hp::init(Vec::new());
        let mut u8_lis: Hp<Vec<u8>> = Hp::init(Vec::new());
        let num_threads = 10;

        // Insert and print num_threads elements concurrently.
        for i in range(0, num_threads) {
            let cint_lis = int_lis.clone();
            let cu8_lis = u8_lis.clone();

            TaskBuilder::new().native().spawn(proc() {
                let mut bint_lis = cint_lis.clone();
                let mut bu8_lis = cu8_lis.clone();

                // Generate a pointer that can safely read
                // a Hp<T> state.
                let mut readint_ptr = bint_lis.get_reader();
                let mut readu8_ptr = bu8_lis.get_reader();
                
                loop {
                    // Read in the current state of Hp<T> and acquire a
                    // local verison of Hp<T> so we can modify the structure.
                    match bint_lis.read(&mut readint_ptr) {
                        Ok( _ ) => (),
                        Err(err) => fail!(err)
                    };

                    // Create a new local state.
                    let mut newint_vec = readint_ptr.clone();
                    
                    // Modify the local state.
                    newint_vec.push(i);
                    
                    // Attempt to update global state with the local one.
                    match bint_lis.replace(&mut readint_ptr, newint_vec) {
                        Ok(_) => break,
                        _ => ()
                    };

                } loop {
                    match bu8_lis.read(&mut readu8_ptr) {
                        Ok( _ ) => (),
                        Err(err) => fail!(err)
                    };

                    let mut newu8_vec = readu8_ptr.clone();
                    
                    newu8_vec.push(i as u8);

                    match bu8_lis.replace(&mut readu8_ptr, newu8_vec) {
                        Ok(_) => break,
                        _ => ()
                    };
                }
        ...
        // Read in current vec list
        let mut readint_ptr = int_lis.get_reader();
        let mut readu8_ptr = u8_lis.get_reader();
        
        // Wait until all threads have made their writes
        loop {
            match int_lis.read(&mut readint_ptr)  {
                Ok( hp ) => hp,
                Err(err) => fail!(err)
            };

            match u8_lis.read(&mut readu8_ptr)  {
                Ok( hp ) => hp,
                Err(err) => fail!(err)
            };

            // There should exist num_threads elements in the vec
            if readu8_ptr.len() == num_threads as uint 
            && readint_ptr.len() == num_threads as uint { break; }
        }
    }*/
}
